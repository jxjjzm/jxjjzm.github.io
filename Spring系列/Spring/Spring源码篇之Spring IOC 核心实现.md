...
进入DefaultBeanDefinitionDocumentReader后，发现这个方法的重要目的之一就是提取Root，以便于再次将root作为参数继续BeanDefinition的注册。经过艰难险阻，磕磕绊绊，我们终于到了核心逻辑的底部 doRegisterBeanDefinitions(root),至少我们在这个方法中看到了希望，如果说以前一直是XML加载解析的准备，那么doRegisterBeanDefinitions(root)算是真正地开始进行解析了。别急，在进行解析之前请允许笔者插播几条说明：
